...
class MatureDimRecorder:
    def __init__(
        ...
        audio_threshold_ratio=0.30,
        video_threshold_ratio=0.30,
        audio_fixed_threshold_quantile=0.80,
        video_fixed_threshold_quantile=0.80,
    ):
        ...
        self.audio_threshold_ratio = float(audio_threshold_ratio)
        self.video_threshold_ratio = float(video_threshold_ratio)
        self.audio_fixed_threshold_quantile = float(audio_fixed_threshold_quantile)
        self.video_fixed_threshold_quantile = float(video_fixed_threshold_quantile)
    ...
    def _freeze_thresholds_if_needed(self):
        ...
        if self.fixed_threshold_mode == "warmup_quantile":
            self.audio_threshold = float(np.quantile(audio_pool, self.audio_fixed_threshold_quantile))
            self.video_threshold = float(np.quantile(video_pool, self.video_fixed_threshold_quantile))
        else:
            self.audio_threshold = self.audio_threshold_ratio * float(audio_pool.max())
            self.video_threshold = self.video_threshold_ratio * float(video_pool.max())
    ...
    def update(self, audio_feat: np.ndarray, video_feat: np.ndarray, labels: np.ndarray):
        ...
        if self.threshold_frozen:
            ...
            if self.fixed_threshold_mode == "warmup_quantile":
                # warmup阶段还没冻结时，先用当前模态自身分位数作为临时阈值
                th_a = float(np.quantile(score_a, self.audio_fixed_threshold_quantile))
                th_v = float(np.quantile(score_v, self.video_fixed_threshold_quantile))
            else:
                th_a = self.audio_threshold_ratio * float(score_a.max())
                th_v = self.video_threshold_ratio * float(score_v.max())
    ...
    def plot_curve(self):
        ...
        meta = {
            ...
            "audio_threshold_ratio": self.audio_threshold_ratio,
            "video_threshold_ratio": self.video_threshold_ratio,
            "audio_fixed_threshold_quantile": self.audio_fixed_threshold_quantile,
            "video_fixed_threshold_quantile": self.video_fixed_threshold_quantile,
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mature_save_root', type=str, default='/data/zyh/NeurIPS24-LFM/_figure/mature_fixed_sepmod')
    ...
    parser.add_argument('--audio_mature_threshold_ratio', type=float, default=0.30,
                        help='Used only when fixed_threshold_mode=warmup_max')
    parser.add_argument('--video_mature_threshold_ratio', type=float, default=0.30,
                        help='Used only when fixed_threshold_mode=warmup_max')
    parser.add_argument('--audio_mature_fixed_threshold_quantile', type=float, default=0.80)
    parser.add_argument('--video_mature_fixed_threshold_quantile', type=float, default=0.80)
    ...
    mature_recorder = MatureDimRecorder(
        ...
        audio_threshold_ratio=args.audio_mature_threshold_ratio,
        video_threshold_ratio=args.video_mature_threshold_ratio,
        audio_fixed_threshold_quantile=args.audio_mature_fixed_threshold_quantile,
        video_fixed_threshold_quantile=args.video_mature_fixed_threshold_quantile,
    )
    logger.info(f'Mature-dimension plots will be saved to: {mature_recorder.save_dir}')
    logger.info(
        f'Separate-modality fixed-threshold mode: {args.mature_fixed_threshold_mode}, '
        f'warmup_epochs={args.mature_warmup_epochs}, '
        f'audio_ratio={args.audio_mature_threshold_ratio}, '
        f'video_ratio={args.video_mature_threshold_ratio}, '
        f'audio_quantile={args.audio_mature_fixed_threshold_quantile}, '
        f'video_quantile={args.video_mature_fixed_threshold_quantile}'
    )
