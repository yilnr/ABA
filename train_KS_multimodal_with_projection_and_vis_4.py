...
def save_epoch_projection_figure(epoch, feat_dict, class_ids, save_dir, logger=None, perplexity=30, seed=42):
    ...
    # 1) Audio 空间：原始 audio + projected video->audio 一起拟合 t-SNE
    audio_all = np.concatenate([audio, proj_v2a], axis=0)
    tsne_audio = TSNE(
        n_components=2,
        perplexity=min(perplexity, max(5, len(audio_all) - 1)),
        learning_rate='auto',
        init='pca',
        random_state=seed,
    )
    audio_all_2d = tsne_audio.fit_transform(audio_all)
    audio_2d = audio_all_2d[:len(audio)]
    proj_v2a_2d = audio_all_2d[len(audio):]

    # 2) Video 空间：原始 video + projected audio->video 一起拟合 t-SNE
    video_all = np.concatenate([video, proj_a2v], axis=0)
    tsne_video = TSNE(
        n_components=2,
        perplexity=min(perplexity, max(5, len(video_all) - 1)),
        learning_rate='auto',
        init='pca',
        random_state=seed,
    )
    video_all_2d = tsne_video.fit_transform(video_all)
    video_2d = video_all_2d[:len(video)]
    proj_a2v_2d = video_all_2d[len(video):]

    # 柔和配色，接近你给的参考风格
    class_colors = {
        class_ids[0]: '#F28E8C',   # 柔和红
        class_ids[1]: '#97D5C9',   # 柔和青绿
    }
    ...
    # (3) audio space + projected video->audio
    draw_pretty_panel(
        axes[2],
        X_base=audio_2d,
        labels_base=labels,
        class_ids=class_ids,
        title='Audio Space w/ Projected Video',
        class_colors=class_colors,
        X_proj=proj_v2a_2d,
        labels_proj=labels,
        proj_name='Video→Audio',
    )

    # (4) video space + projected audio->video
    draw_pretty_panel(
        axes[3],
        X_base=video_2d,
        labels_base=labels,
        class_ids=class_ids,
        title='Video Space w/ Projected Audio',
        class_colors=class_colors,
        X_proj=proj_a2v_2d,
        labels_proj=labels,
        proj_name='Audio→Video',
    )
    ...
    handles = [
        plt.Line2D([0], [0], marker='o', color='w',
                   ...
                   markersize=8, alpha=0.85, label=f'Class {class_ids[0]}'),
        plt.Line2D([0], [0], marker='o', color='w',
                   markerfacecolor=class_colors[class_ids[1]],
                   markeredgecolor='white', markeredgewidth=0.8,
                   markersize=8, alpha=0.85, label=f'Class {class_ids[1]}'),
        plt.Line2D([0], [0], marker='o', color='w',
                   markerfacecolor='#F4D4D3',
                   markeredgecolor='#B85C5C',
                   markeredgewidth=1.0,
                   markersize=8, alpha=0.95, label='Projected samples'),
    ]
    ...
    png_path = os.path.join(save_dir, f'epoch_{epoch:03d}_projection_view_4panel.png')
    pdf_path = os.path.join(save_dir, f'epoch_{epoch:03d}_projection_view_4panel.pdf')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_root', type=str, default='/data/zyh/NeurIPS24-LFM/_tsne/projection4')
