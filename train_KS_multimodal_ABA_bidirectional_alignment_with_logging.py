...
def write_tensorboard_scalars(writer, epoch, train_metrics, val_metrics):
    ...
    for key, value in train_metrics.items():
        tag_prefix = 'ABA' if key in ['mass_a2v', 'mass_v2a', 'lambda_a2v', 'lambda_v2a', 'valid_classes_per_batch', 'beta'] else 'Train'
...
class AdaptiveBidirectionalAligner(nn.Module):
    ...
    Practical implementation of the paper idea for audio-video training:
      ...
      3) adaptive transported mass and adaptive directional weighting from unimodal confidence.
    ...
    def forward(self, feat_a, feat_v, logits_a, logits_v, y):
        ...
        for cls in labels.unique(sorted=False).tolist():
            ...
            # Class-wise learning confidence, following the paper's learning-status signal.
            s_a = prob_a[mask, cls].mean()
            s_v = prob_v[mask, cls].mean()

            mass_logits = torch.stack([s_a / max(self.tau_m, self.eps),
                                       s_v / max(self.tau_m, self.eps)])
            mass_weights = F.softmax(mass_logits, dim=0)
            m_a2v = self.rho * mass_weights[0]
            m_v2a = self.rho * mass_weights[1]
...
if __name__ == '__main__':
    ...
    logger.info(
        ...
        f'tau_m={args.aba_tau_m}, tau_lambda={args.aba_tau_lambda}, '
