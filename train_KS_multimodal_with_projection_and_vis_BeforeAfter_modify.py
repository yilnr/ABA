...
def draw_before_after_panel(ax,
    ...
    # before：未投影源模态。
    # 相比上一版，这里把 before 进一步加深，便于观察未投影源模态的位置。
    for cls in class_ids:
        ...
        ax.scatter(
            ...
            s=32,
            c=[lighten_color(class_colors[cls], factor=0.45)],
            alpha=0.66,
            edgecolors='none',
            zorder=1,
        )
    ...
    # after：投影后的样本。
    # 相比上一版，after 的填充色和边框都进一步加深，突出投影后的样本。
    for cls in class_ids:
        ...
        ax.scatter(
            ...
            alpha=0.92,
            edgecolors=[lighten_color(class_colors[cls], factor=0.02)],
            linewidths=1.05,
            zorder=3,
        )
...
def save_epoch_projection_figure(epoch, feat_dict, class_ids, save_dir, logger=None, perplexity=30, seed=42):
    ...
    handles = [
        ...
        plt.Line2D([0], [0], marker='o', color='w',
                   markerfacecolor='#AFAFAF',
                   markeredgecolor='none',
                   markersize=7, alpha=0.95, label='Before'),
        plt.Line2D([0], [0], marker='o', color='w',
                   markerfacecolor='#E7847F',
                   markeredgecolor='#9F3F3F',
                   markeredgewidth=1.1,
                   markersize=8, alpha=0.96, label='After (projected)'),
    ]
    fig.legend(
        handles=handles,
        loc='lower center',
...
if __name__ == '__main__':
    ...
    logger.info(f'Selected fixed classes for visualization: {selected_classes}')
