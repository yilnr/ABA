...
def draw_feature_panel(ax, X2d, labels, class_ids, title, class_colors, hollow=False, legend_prefix=None):
    ...
    for cls in class_ids:
        ...
        if hollow:
            ax.scatter(
                ...
                s=28, facecolors='none', edgecolors=class_colors[cls],
                linewidths=1.1, alpha=0.95,
                label=f"{legend_prefix} class {cls}" if legend_prefix else f"class {cls}"
            )
        else:
            ax.scatter(
                X2d[mask, 0], X2d[mask, 1],
                s=24, c=class_colors[cls], alpha=0.72,
...
def save_epoch_projection_figure(epoch, feat_dict, class_ids, save_dir, logger=None, perplexity=30, seed=42):
    ...
    # Left: audio feature space
    tsne_audio = TSNE(
        n_components=2,
        perplexity=min(perplexity, max(5, len(audio) - 1)),
        learning_rate='auto',
        init='pca',
        random_state=seed,
    )
    audio_2d = tsne_audio.fit_transform(audio)

    # Right: target video space, jointly embed original video and projected audio->video
    right_all = np.concatenate([video, proj_a2v], axis=0)
    right_labels = np.concatenate([labels, labels], axis=0)
    tsne_video = TSNE(
        n_components=2,
        perplexity=min(perplexity, max(5, len(right_all) - 1)),
        learning_rate='auto',
        init='pca',
        random_state=seed,
    )
    right_2d = tsne_video.fit_transform(right_all)
    video_2d = right_2d[: len(video)]
    proj_2d = right_2d[len(video):]
    ...
    draw_feature_panel(
        axes[0], audio_2d, labels, class_ids,
    ...
    draw_feature_panel(
        axes[1], video_2d, labels, class_ids,
    ...
    draw_feature_panel(
        axes[1], proj_2d, labels, class_ids,
        title='Video Feature Space',
        class_colors=class_colors,
        hollow=True,
        legend_prefix='Projected'
    )
    axes[1].text(0.03, 0.96, 'Projected Audio→Video', transform=axes[1].transAxes,
                 fontsize=11, weight='bold', va='top')

    handles = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=class_colors[class_ids[0]],
                   markeredgecolor='white', markersize=8, label=f'Class {class_ids[0]}'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=class_colors[class_ids[1]],
                   markeredgecolor='white', markersize=8, label=f'Class {class_ids[1]}'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='none', markeredgecolor='#666666',
                   markersize=8, label='Projected Audio→Video'),
    ]
    fig.legend(handles=handles, loc='lower center', ncol=3, frameon=False, fontsize=11, bbox_to_anchor=(0.5, -0.02))
    fig.suptitle(f'Epoch {epoch}', fontsize=16, weight='bold', y=1.02)
    plt.tight_layout(rect=[0, 0.06, 1, 0.98])

    png_path = os.path.join(save_dir, f'epoch_{epoch:03d}_projection_view.png')
    pdf_path = os.path.join(save_dir, f'epoch_{epoch:03d}_projection_view.pdf')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_root', type=str, default='/data/zyh/NeurIPS24-LFM/_tsne/projection2')
